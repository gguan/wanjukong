...
const STRINGS: Record<SupportedLocale, Strings> = {
  en: {
    ...
    preheader: `Use this link to reset your ${BRAND} password.`,
  },
  ja: {
    subject: 'パスワードの再設定',
    heading: 'パスワードを再設定する',
    ...
    preheader: `このリンクから ${BRAND} のパスワードを再設定できます。`,
  },
  'zh-CN': {
    subject: '重置您的密码',
    heading: '重置您的密码',
...
export function getPasswordResetEmail(
  ...
  const body = `
    ...
    <p style="margin:0 0 20px;font-size:14px;line-height:1.5;color:#71717a;word-break:break-all;">${resetUrl}</p>
